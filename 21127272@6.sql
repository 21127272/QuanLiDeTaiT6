...
FROM GIAOVIEN GV
WHERE NOT EXISTS (SELECT GV1.LUONG
				FROM GIAOVIEN GV1
...
FROM GIAOVIEN GV
WHERE GV.MABM = 'HTTT'
AND NOT EXISTS (SELECT GV1.LUONG
			FROM GIAOVIEN GV1
			WHERE GV1.LUONG > GV.LUONG)
...
FROM GIAOVIEN GV
WHERE GV.MABM = 'HTTT'
AND GV.NGSINH <= ALL(SELECT GV.NGSINH
					FROM GIAOVIEN GV
					WHERE GV.MABM = 'HTTT')
...
--Q40
SELECT DISTINCT GV.HOTEN, GV.LUONG, BM.MAKHOA
FROM GIAOVIEN GV, BOMON BM, KHOA K
...
AND NOT EXISTS (SELECT GV1.LUONG
				...
				AND GV1.LUONG > GV.LUONG)

--Q42
SELECT DISTINCT DT.TENDT
FROM THAMGIADT TGDT, GIAOVIEN GV, DETAI DT
...
--Q43
SELECT DISTINCT DT.TENDT, GV1.HOTEN
FROM THAMGIADT TGDT, GIAOVIEN GV, DETAI DT, GIAOVIEN GV1
WHERE DT.MADT NOT IN (SELECT DT.MADT
					FROM DETAI DT, GIAOVIEN GV, THAMGIADT TGDT
					WHERE GV.HOTEN = N'Nguyễn Hoài An'
					AND GV.MAGV = TGDT.MAGV
...
WHERE EXISTS (SELECT GV1.MAGV
			FROM GIAOVIEN GV1
			WHERE GV.PHAI = GV1.PHAI
			AND GV.HOTEN = GV1.HOTEN
			AND GV.MAGV != GV1.MAGV)

--Q49
SELECT DISTINCT GV.*
FROM GIAOVIEN GV, BOMON BM
WHERE BM.TENBM = N'Công nghệ phần mềm'
AND EXISTS(SELECT GV1.LUONG
			FROM GIAOVIEN GV1
			WHERE GV.LUONG > GV1.LUONG
...
--Q51
SELECT K.TENKHOA, COUNT(GV.MAGV) AS N'Số lượng giáo viên'
FROM KHOA K, GIAOVIEN GV, BOMON BM
WHERE GV.MABM = BM.MABM
AND BM.MAKHOA = K.MAKHOA
GROUP BY K.TENKHOA
HAVING COUNT(GV.MAGV) >= ALL(SELECT COUNT(GV1.MAGV)
							FROM KHOA K, GIAOVIEN GV1, BOMON BM
							WHERE GV1.MABM = BM.MABM
							AND BM.MAKHOA = K.MAKHOA
							GROUP BY K.TENKHOA)

--Q52
SELECT GV.HOTEN, COUNT(DT.MADT) AS N'Số lượng đề tài'
...
--Q53
SELECT BM.MABM, COUNT(GV.MAGV) AS 'Số lượng giáo viên'
...
--Q54
SELECT GV.HOTEN, BM.TENBM, COUNT(TGDT.MADT) AS 'Số lượng đề tài'
FROM GIAOVIEN GV, BOMON BM, THAMGIADT TGDT
WHERE GV.MABM = BM.MABM
AND TGDT.MAGV = GV.MAGV
GROUP BY GV.HOTEN, BM.TENBM
HAVING COUNT(TGDT.MADT) >= ALL (SELECT COUNT(TGDT1.MADT)
								FROM GIAOVIEN GV1, BOMON BM1, THAMGIADT TGDT1
								WHERE GV1.MABM = BM1.MABM
								AND TGDT1.MAGV = GV1.MAGV
								GROUP BY GV1.HOTEN, BM1.TENBM)

--Q55
SELECT GV.HOTEN, COUNT(TGDT.MADT) AS 'Số lượng đề tài'
FROM GIAOVIEN GV, THAMGIADT TGDT
WHERE GV.MABM = 'HTTT'
AND GV.MAGV = TGDT.MAGV
GROUP BY GV.HOTEN
HAVING COUNT(TGDT.MAGV) >= ALL(SELECT COUNT(TGDT1.MADT)
							FROM GIAOVIEN GV1, THAMGIADT TGDT1
							WHERE GV1.MABM = 'HTTT'
							AND GV1.MAGV = TGDT1.MAGV
							GROUP BY GV1.HOTEN)

--Q56
SELECT GV.HOTEN, BM.TENBM, COUNT(NT.MAGV) AS 'Số người thân'
FROM GIAOVIEN GV, BOMON BM, NGUOITHAN NT
WHERE GV.MABM = BM.MABM
AND NT.MAGV = GV.MAGV
GROUP BY GV.HOTEN, BM.TENBM
HAVING COUNT(NT.MAGV) >= ALL(SELECT COUNT(NT1.MAGV)
							FROM GIAOVIEN GV1, BOMON BM1, NGUOITHAN NT1
							WHERE GV1.MABM = BM1.MABM
							AND NT1.MAGV = GV1.MAGV
							GROUP BY GV1.HOTEN, BM1.TENBM)

--Q57
SELECT GV.HOTEN, COUNT(DT.GVCNDT) AS 'Số lượng đề tài chủ nhiệm'
FROM GIAOVIEN GV, DETAI DT, BOMON BM
WHERE GV.MAGV = DT.GVCNDT
AND BM.TRUONGBM = GV.MAGV
GROUP BY GV.HOTEN
